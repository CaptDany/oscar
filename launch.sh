...
tmux send-keys -t $SESSION_NAME:1.1 "clear && echo -e '${PURPLE}=== MINIO ===${NC}' && (docker run -d --name oscar-minio -p 9000:9000 -p 9001:9001 -e MINIO_ROOT_USER=minioadmin -e MINIO_ROOT_PASSWORD=minioadmin minio/minio server /data --console-address ':9001' 2>/dev/null || echo 'Docker not available') && sleep 1 && clear && echo -e '${PURPLE}=== MAILHOG ===${NC}' && (docker run -d --name oscar-mailhog -p 1025:1025 -p 8025:8020 mailhog/mailhog 2>/dev/null || echo 'Docker not available')" C-m
